...
public class Surv1_Controller : MonoBehaviour
{
    [SerializeField] Player_Stat playerstat;
    public Transform player; // 플레이어의 위치를 저장
    public float followDistance = 3f; // 플레이어와 엔지니어 사이의 거리 임시값   
    ...
    void FollowPlayer()
    {
        ...
        float distance = Vector2.Distance(transform.position, player.position);
            ...
            Vector2 targetPosition = (Vector2)player.position - inputDirection * 1f;

        // 엔지니어를 목표 위치로 이동
        transform.position = (Vector2)player.position - inputDirection * 1f;
    }
}
