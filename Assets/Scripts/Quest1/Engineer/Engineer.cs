...
public class Engineer : PlayerController
{
    ...
    /*
     * 굳이 isInteractionStarted를 사용해야할까???????????
     */
    ...
    private void Update()
    {
        ...
        if (!following && engineerPlayerDistance <= 3f) // 엔지니어 구출 조건
        {
            ...
            else if (isInteractionStarted && (interactPlayerPosition != player_T.position) )                                                       // 조건 변경해야함 : 플레이어의 체력이 줄어든 경우!!
            {
        ...
        if (following)  // 엔지니어가 플레이어를 따라다닐 경우 : 플레이어가 시작위치로 가면 Quest1을 클리어하고 Map2로 넘어가서 Quest2를 실행하도록
        {
            ...
            if ((Vector3.Distance(questFinVector, player_T.position)) < 3f) // 퀘스트1 완료 조건
            {
                Debug.Log("퀘스트1 클리어!");
